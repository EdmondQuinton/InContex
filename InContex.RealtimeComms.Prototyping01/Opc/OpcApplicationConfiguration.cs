...
namespace InContex.RealtimeComms.Prototyping01.Opc
{
    public class ModuleConfiguration
    {
        ...
        public ModuleConfiguration(string applicationName)
        {
            ...
            Configuration.ApplicationUri = "urn:" + Utils.GetHostName() + ":InContex:" + Configuration.ApplicationName;
            ...
            try
            {
                ...
                if (store == null)
                {
                    __logger.Log(LogLevel.Error, "Could not open trusted peer store. StorePath={0}", Configuration.SecurityConfiguration.TrustedPeerCertificates.StorePath);
                ...
                else
                {
                    try
                    {
                        __logger.Log(LogLevel.Info, "Adding certificate to trusted peer store. StorePath={0}", Configuration.SecurityConfiguration.TrustedPeerCertificates.StorePath);
            ...
            {
                __logger.Log(LogLevel.Error, e, "Could not add certificate to trusted peer store. StorePath={0}", Configuration.SecurityConfiguration.TrustedPeerCertificates.StorePath);
