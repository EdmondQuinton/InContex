...
{
    /// <summary>
    /// Class provides high speed serialization of primitive types intended for use with the IPPQueue. 
    /// This class does not support all generic struct types and as such is not visible to external 
    /// libraries or code.
    /// </summary>
    /// <typeparam name="T">Primitive type to serialize.</typeparam>
    internal class PrimitiveTypeSerializer<T> : ISerializer<T> where T : struct
    {
        private int _intSize;
        private int _longSize;
        private int _guidSize;

        internal PrimitiveTypeSerializer()
        {
            Type type = typeof(T);
